...
abstract class Model{
    ...
    public const RULE_DURATION="duration";
        ...
        foreach($this->rules() as $attribute=>$rules){
            ...
            foreach($rules as $rule){
                ...
                if($ruleName==self::RULE_DURATION){
                    ...
                    if($value>7){
                        $this->addError($attribute,self::RULE_DURATION);

                    }
                }
                if($ruleName==self::RULE_PERCENTAGE){
                    if($value<0 or $value>100){
                        $this->addError($attribute,self::RULE_PERCENTAGE);
                    }
                }
                ...
                if($ruleName==self::RULE_TIME && isset($rule['start_date']) && isset($rule['date']) && ($rule['start_date']==Date('Y-m-d') && $rule['date']==date('l',strtotime(Date('Y-m-d'))) )  && $value<$time->addTime(Date('H:i'),'00:30')){
    ...
    public function errorMessages(){
        return [
            ...
            self::RULE_PERCENTAGE=>"Wrond Percentage"
